...
Uses OpenAI's Codex CLI (`codex exec --full-auto`) to get a **fresh, independent perspective** from a competing model (GPT-5) on Claude Code infrastructure. This extends the agents-vs-skills principle: not just fresh Claude context, but an entirely different model reviewing the work.
...
1. **Verify Codex is installed:**
   ...
   ```
   If not found, stop and tell the user: "Codex CLI is not installed. Install with `npm install -g @openai/codex` or check your PATH."
...
- **NEVER run git commands that change state** (no commit, add, push, checkout)
...
Compare against the baseline from pre-flight. If there are **new changes not present before**:
...
3. **Do not proceed** with report generation until resolved
...
> Model: GPT-5 | Read-only audit | No files modified.
...
| Situation | Action |
|-----------|--------|
| Codex not installed | Stop, show install instructions |
...
| `/audit-project-structure` | Per-project structural audit — Codex audits are cross-cutting |
