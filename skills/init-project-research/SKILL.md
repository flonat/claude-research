...
7. **HPC scaffold?** — Only ask for Experimental, Computational, or Mixed. "Will this need your HPC cluster (GPU, long sweeps, large state spaces)?" Yes / No / Later. If Yes, scaffold `hpc/` with `submit.sbatch` + `sweep.sbatch` + `sync-up.sh` + `sync-down.sh` + `env-setup.sh` (+ `prestage-models.sh` for LLM projects) from `templates/slurm/` and reference implementations under `Projects/[your-project]/hpc/`. If Later, add `hpc/` to `.gitignore` placeholder — scaffold on demand. See [`docs/guides/hpc.md`](../../docs/guides/hpc.md) in Task Management.
